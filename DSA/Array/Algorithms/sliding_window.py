...
In sliding window we always need to check is our window valid
...
validation condition differs from problem to problem
 ...
 2. once our substr fails the validation , then we will move our l and check the validation again then if valid move R otherwise L 
...
def findLongestSubstring(s: str):
    ...
    for r in range(len(s)):  # O(n) as looping
        # do this until the window is valid , for this case window is valid when it does not have any duplicates
        while s[r] in sett:
            sett.remove(s[l])  # O(1)
            l += 1
